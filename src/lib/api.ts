...
export async function fetchPosts(params?: { page?: number; limit?: number; sort?: "latest" | "views" }) {
  const query = new URLSearchParams();
  if (params?.page) query.set("page", String(params.page));
  if (params?.limit) query.set("limit", String(params.limit));
  if (params?.sort) query.set("sort", params.sort);
...
export async function fetchPostById(postId: string) {
  ...
  const data = await res.json() as { item: ApiPost };
  return data.item;
}
...
export async function fetchPostComments(postId: string) {
  ...
  const data = await res.json() as { items: ApiPostComment[] };
  return data.items;
}
...
export async function addPostComment(postId: string, body: string) {
  ...
  return res.json() as Promise<{ item: ApiPostComment }>;
}

export async function fetchPostDownloadUrl(postId: string) {
  const res = await fetch(`${API_BASE}/posts/${postId}/download-url`);
  if (!res.ok) throw new Error("Failed to get download url");
  const data = await res.json() as { url: string };
  return data.url;
}
